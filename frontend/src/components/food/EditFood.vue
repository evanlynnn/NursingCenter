...
export default {
  ...
  created() {
    // Retrieve edited food data from server
    let foodId = this.$route.params.id
    axios.get('http://localhost:8081/food/get/' + foodId)
        .then((response) => {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      axios.post('http://localhost:8081/food/update', this.formData)
          .then((response) => {
            if (response.data === true) {
              this.$message.success("更新膳食成功")
              this.$router.push("/main/food")
            } else {
